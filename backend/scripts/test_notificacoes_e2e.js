...
async function run() {
  ...
  // POST
  const payload = { instituicao: 'Teste Unit', assunto: 'Teste integração', descricao: 'Corpo da mensagem de teste.' };
  r = await fetch(base + '/notificacoes', { method: 'POST', headers, body: JSON.stringify(payload) });
  ...
  // PATCH
  r = await fetch(`${base}/notificacoes/${encodeURIComponent(newId)}/lida`, { method: 'PATCH', headers });
