...
(async () => {
  ...
  const payload = { tipo: 'MENSAGEM', destinatario: 'Administrador Geral', instituicao: 'Instituição UI Test', assunto: 'Teste UI: via API', descricao: 'Mensagem enviada por script de verificação.' };
  const post = await fetch(base + '/notificacoes', {
    method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + tokenInst },
  ...
  const get = await fetch(base + '/notificacoes?limite=10', { headers: { Authorization: 'Bearer ' + tokenAdmin } });
