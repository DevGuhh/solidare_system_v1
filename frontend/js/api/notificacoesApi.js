...
export async function listarNotificacoes(limite = 10) {
    try {
        const response = await apiClient.get('/notificacoes?limite=' + encodeURIComponent(limite));
...
export async function enviarNotificacao(dados = {}) {
    try {
        ...
        console.warn('Backend de notificações indisponível; usando fallback frontend.', error);
        ...
        try {
            ...
            const novo = {
                id: 'local-' + Date.now(),
                instituicao: dados.instituicao || dados.remetente || 'Instituição (local)',
                ...
                remetente: dados.instituicao || dados.remetente || 'Instituição (local)',
            ...
            return { sucesso: true, mock: true, mensagem: 'Mensagem registrada localmente (falha ao persistir).', ...dados };
        }
    }
}
...
export async function marcarComoLida(id) {
    try {
        const response = await apiClient.patch(`/notificacoes/${encodeURIComponent(id)}/lida`);
