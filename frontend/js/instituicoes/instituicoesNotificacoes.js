...
export function initFormNotificacao() {
  ...
  async function renderThread() {
    ...
    const userIdEl =
      document.getElementById("instituicaoId") ||
      document.querySelector("[data-instituicao-id]") ||
      document.querySelector(".usuario-info [data-id]");
    const userId =
      userIdEl?.value ||
      userIdEl?.dataset?.instituicaoId ||
      userIdEl?.dataset?.id ||
      null;
    const userEmail =
    ...
    try {
      ...
      relacionados
        ...
        .forEach((m) => {
          const isUsuario =
            String(m.remetenteId) === String(userId) ||
            (!m.remetenteId && String(m.remetente).includes(userEmail));
          const senderName = isUsuario
            ? m.remetente || m.instituicao || m.destinatario
            : m.remetente || m.instituicao || m.destinatario;
          const hora = new Date(
            m.data || m.criadoEm || m.lidaEm || Date.now(),
          ...
          bubble.className =
            "mensagem-bubble " + (isUsuario ? "bubble-left" : "bubble-right");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const descricao = document
      .getElementById("notificacaoDescricao")
      ?.value.trim();
    ...
    const userId2 =
      userIdEl2?.value ||
      userIdEl2?.dataset?.instituicaoId ||
      userIdEl2?.dataset?.id ||
      null;
    ...
    const payload = {
      ...
      destinatarioId: null,
      instituicao:
        document.querySelector(".usuario-info strong")?.textContent?.trim() ||
        null,
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("solidare:notificacao", {
          detail: notificacao,
        }),
      );
